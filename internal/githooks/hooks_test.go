...
func TestInstallAndUninstallHookBlock(t *testing.T) {
	...
	installed, err := Install(root, []string{"pre-commit"}, Options{Command: "stack scan --exit-code"})
	if err != nil {
	...
	content := string(data)
	if !strings.Contains(content, "echo existing") || !strings.Contains(content, startMarker) || !strings.Contains(content, "stack scan --exit-code") {
